...
if [ ! -f "$PROJECT_ROOT/manage.py" ]; then
    ...
    echo "Make sure this script is being run from inside the legacylens project structure"
    exit 1
fi
...
if [ -z "$DJANGO_SETTINGS_MODULE" ]; then
    export DJANGO_SETTINGS_MODULE=legacylens.settings
...
if [ -n "$1" ]; then
    ...
    python manage.py test "legacylens.apps.documents.tests.test_document_service.TestDocumentServiceIntegration.$1" -v 2
else
    # Run all document service tests
    echo "Running all document service tests"
    python manage.py test legacylens.apps.documents.tests.test_document_service -v 2
fi
...
if [ $TEST_RESULT -eq 0 ]; then
    ...
    echo "    },"
    echo "    'loggers': {"
    echo "        'legacylens.apps.documents': {"
